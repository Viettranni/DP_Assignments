...
public class Gui extends Application {
    ...
    public void start(Stage stage) {
        ...
        Label y_label = new Label("Press Ctrl-Y to redo the last undone.");
        y_label.setPadding(insets);

        // Create a Button for History View
        Button openButton = new Button("Open New Window");
        openButton.setOnAction(e -> openNewWindow());

        // create a VBox that contains the HBox and the CheckBox
        VBox vBox = new VBox(hBox, checkBox, label, y_label, openButton);
    ...
    private void openNewWindow() {
        ...
        for (IMemento memento : controller.getHistoryList()) {
            historyListView.getItems().add(memento.toString()); 

        historyListView.getSelectionModel().selectedItemProperty().addListener((observable, oldValue, newValue) -> {
            if (newValue != null) {
                IMemento selectedMemento = controller.getHistoryList().get(historyListView.getSelectionModel().getSelectedIndex());
        ...
        VBox historyLayout = new VBox(historyListView, closeButton);
        historyLayout.setSpacing(10);
        Scene secondScene = new Scene(historyLayout, 300, 250);

        newWindow.setTitle("History View");
